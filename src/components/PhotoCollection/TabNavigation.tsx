...
const PhotoCollectionTabNavigation: React.FC<Props> = ({tabs}) => {
    ...
    return (
        <div className="bg-active-theme/90 top-site-header-scrolled sticky z-50 backdrop-blur-2xl will-change-transform lg:-mx-8">
                ...
                <ul className="flex w-full flex-row gap-1 py-1">
                    {tabs.map(tab => (
                        <li key={tab.id} className="flex-grow">
                            <Link
                                className={clsx(
                                    'block rounded py-2 text-center text-sm font-medium underline-offset-2 transition-colors duration-200 ease-in-out',
                                    {
