...
const ThumbnailImage: React.FC<Props> = ({base64, fill, height, loading = 'lazy', width, url}) => (
    <span
        className={clsx(
            'relative block min-h-[50px] overflow-hidden bg-[var(--photo-bg-color)] transition duration-200 ease-in-out hover:duration-500 group-focus:outline-none dark:bg-[var(--dark-photo-bg-color)] dark:group-focus:ring-white',
            {'h-full': fill}
        )}
    >
        <Image
